...
pub use line_emit::{PTabGeometry, PTabPlacement, resolve_ptab};
...
use borders::{SegmentEdges, emit_paragraph_borders_and_shading, emit_segment_borders_and_shading};
...
impl PlacedParagraph<'_> {
    ...
    fn emit_drop_cap(&self, commands: &mut Vec<DrawCommand>) {
        if let (Some(dc), Some(baseline_y)) = (&self.style.drop_cap, self.drop_cap_baseline_y) {
