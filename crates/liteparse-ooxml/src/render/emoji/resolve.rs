...
mod tests {
    ...
    /// Build a real `TypefaceEntry` from any system font. Used by tests that
    /// need the resolved variant to carry a real face handle.
    fn any_typeface_entry() -> TypefaceEntry {
        ...
        let id = registry
            ...
            .faces()
            .next()
            .expect("system has no fonts — cannot run test")
            .id;
        TypefaceEntry {
            id,
