...
use crate::docx::parse::primitives::{HexColor, OnOff, last_toggle};
...
pub(crate) struct RPrXml {
    ...
    highlight: Option<StHighlightColor>,
    #[serde(default)]
    shd: Option<ShdXml>,
    ...
    vert_align: Option<StVerticalAlignRun>,
    ...
    spacing: Option<Dimension<Twips>>,
    ...
    position: Option<Dimension<HalfPoints>>,
...
impl RPrXml {
    ...
    pub(crate) fn split(self) -> (RunProperties, Option<StyleId>) {
        ...
        let props = RunProperties {
            ...
            font_size: self.sz.and_then(|s| s.val),
            ...
            kerning: self.kern.and_then(|k| k.val),
...
mod tests {
    ...
    #[test]
    fn valid_font_size_still_parses() {
        let (rp, _) = parse(r#"<rPr><sz val="24"/></rPr>"#);
        assert_eq!(rp.font_size.map(|d| d.raw()), Some(24));
    }
