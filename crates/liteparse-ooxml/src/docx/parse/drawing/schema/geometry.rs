...
impl From<CustomGeometryXml> for CustomGeometry {
    fn from(x: CustomGeometryXml) -> Self {
        Self {
            ...
            ah_list: x
                .ah_lst
                .map(|a| a.handles.into_iter().filter_map(to_adjust_handle).collect())
                .unwrap_or_default(),
            cxn_list: x
                .cxn_lst
...
/// An unmodelled adjust-handle element is dropped by the caller; `AdjustHandle`
/// has no variant that could stand in for one.
fn to_adjust_handle(h: AhXml) -> Option<AdjustHandle> {
    Some(match h {
        AhXml::Other => return None,
        AhXml::XY(x) => AdjustHandle::XY {
        ...
        AhXml::Polar(p) => AdjustHandle::Polar {
...
fn to_command(c: PathCommandXml) -> Option<PathCommand> {
    Some(match c {
        // An unmodelled path command is dropped, as malformed ones already are.
        PathCommandXml::Other => return None,
