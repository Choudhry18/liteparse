...
pub struct PositionXml {
    ...
    pub relative_from: Option<StRelFrom>,
...
pub struct AlignXml {
    ...
    pub value: Option<StAnchorAlignment>,
}
...
fn position(p: Option<PositionXml>) -> AnchorPosition {
    ...
    let rel = p.relative_from.map_or(AnchorRelativeFrom::Page, Into::into);
    if let Some(a) = p.align {
        return AnchorPosition::Align {
            relative_from: rel,
            alignment: a.value.map_or(AnchorAlignment::Left, Into::into),
        };
    }
...
mod tests {
    ...
    fn negative_decimal_extent_degrades_to_zero() {
        ...
        assert_eq!(parsed.cx.raw(), 0, "negative extent must not become a size");
        assert_eq!(parsed.cy.raw(), 100, "the valid axis must be unaffected");
    }
