...
/// Convert a color-choice element, or `None` if it is one we don't model.
///
/// Vendoring divergence from dxpdf: upstream is an infallible `From`, which is
/// only possible while every color element is known. With an `Other` catch-all
/// there is no honest `DrawingColor` to return for an unknown element — the
/// model has no "unspecified" variant — and inventing one (black) would be
/// silent corruption. Callers drop the color instead.
pub(crate) fn to_drawing_color(c: DrawingColorXml) -> Option<DrawingColor> {
    Some(match c {
...
pub struct SysClrXml {
    ...
    pub val: Option<StSystemColorVal>,
    #[serde(
        rename = "@lastClr",
        default,
        deserialize_with = "crate::docx::parse::primitives::lenient::opt_attr"
...
pub struct SchemeClrXml {
    ...
    pub val: Option<StSchemeColorVal>,
    #[serde(rename = "$value", default)]
    pub transforms: Vec<ColorTransformXml>,
}
...
pub struct PrstClrXml {
    ...
    pub val: Option<StPresetColorVal>,
    #[serde(rename = "$value", default)]
    pub transforms: Vec<ColorTransformXml>,
}
...
fn convert_transforms(xs: Vec<ColorTransformXml>) -> Vec<ColorTransform> {
    xs.into_iter().filter_map(to_color_transform).collect()
}

/// Convert one color transform, or `None` for an element we don't model.
///
/// Dropping an unknown transform is the right degradation: transforms compose
/// in document order, so omitting one leaves the others applied rather than
/// discarding the whole color.
fn to_color_transform(c: ColorTransformXml) -> Option<ColorTransform> {
    use ColorTransform as Self_;
    ...
    Some(match c {
        ...
        X::Gray => Self_::Gray,
        X::Gamma => Self_::Gamma,
        X::InvGamma => Self_::InvGamma,
        X::Tint(v) => Self_::Tint(v.val),
        X::Shade(v) => Self_::Shade(v.val),
        X::Alpha(v) => Self_::Alpha(v.val),
        X::AlphaOff(v) => Self_::AlphaOff(v.val),
        X::AlphaMod(v) => Self_::AlphaMod(v.val),
        X::HueMod(v) => Self_::HueMod(v.val),
        X::Sat(v) => Self_::Sat(v.val),
        X::SatOff(v) => Self_::SatOff(v.val),
        X::SatMod(v) => Self_::SatMod(v.val),
        X::Lum(v) => Self_::Lum(v.val),
        X::LumOff(v) => Self_::LumOff(v.val),
        X::LumMod(v) => Self_::LumMod(v.val),
        X::Red(v) => Self_::Red(v.val),
        X::RedOff(v) => Self_::RedOff(v.val),
        X::RedMod(v) => Self_::RedMod(v.val),
        X::Green(v) => Self_::Green(v.val),
        X::GreenOff(v) => Self_::GreenOff(v.val),
        X::GreenMod(v) => Self_::GreenMod(v.val),
        X::Blue(v) => Self_::Blue(v.val),
        X::BlueOff(v) => Self_::BlueOff(v.val),
        X::BlueMod(v) => Self_::BlueMod(v.val),
        X::Hue(v) => Self_::Hue(v.val),
        X::HueOff(v) => Self_::HueOff(v.val),
    })
}
...
mod tests {
    ...
    fn parse(xml: &str) -> DrawingColor {
        ...
        to_drawing_color(c).expect("test fixture uses a modelled color element")
    }
    ...
    /// Upstream asserted that an unknown preset color is a hard error. We
    /// deliberately reversed that: an unrecognized value must degrade to "no
    /// color" rather than abort the document. The color element is dropped,
    /// and the containing fill/effect drops with it.
    #[test]
    fn unknown_preset_color_drops_the_color_instead_of_failing() {
        let x: PrstClrXml = quick_xml::de::from_str(r#"<x val="unknownColor"/>"#)
            .expect("an unknown preset color must not fail the parse");
        ...
        let w: Wrap = quick_xml::de::from_str(r#"<wrap><prstClr val="unknownColor"/></wrap>"#)
            .expect("an unknown preset color must not fail the parse");
        assert!(to_drawing_color(w.c).is_none());
    }

    /// A known preset still resolves — the leniency must not swallow good input.
    #[test]
    fn known_preset_color_still_parses() {
        let x: PrstClrXml =
            quick_xml::de::from_str(r#"<x val="red"/>"#).expect("known preset must parse");
