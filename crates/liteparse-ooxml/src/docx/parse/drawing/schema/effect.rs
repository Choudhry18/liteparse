...
use super::color::{DrawingColorXml, to_drawing_color};
...
pub struct PrstShdwXml {
    ...
    pub prst: Option<StPresetShadowVal>,
    #[serde(
        rename = "@dist",
        default,
        deserialize_with = "deserialize_optional_nonnegative_dimension"
...
impl TryFrom<EffectXml> for Effect {
    ...
    fn try_from(e: EffectXml) -> Result<Self, Self::Error> {
        Ok(match e {
            // Unmodelled effect element — reported like a malformed one.
            EffectXml::Other => return Err("unknown"),
            ...
            EffectXml::FillOverlay(f) => {
                ...
                Effect::FillOverlay(FillOverlayEffect {
                    fill,
                    blend: f.blend.ok_or("fillOverlay")?.into(),
                })
            }
            EffectXml::Glow(g) => Effect::Glow(GlowEffect {
                radius: g.rad.unwrap_or_default(),
                color: to_drawing_color(g.color.ok_or("glow")?).ok_or("glow")?,
            }),
            EffectXml::InnerShdw(i) => {
                let color: DrawingColor =
                    to_drawing_color(i.color.ok_or("innerShdw")?).ok_or("innerShdw")?;
            ...
            EffectXml::OuterShdw(o) => {
                let color: DrawingColor =
                    to_drawing_color(o.color.ok_or("outerShdw")?).ok_or("outerShdw")?;
            ...
            EffectXml::PrstShdw(p) => {
                let color: DrawingColor =
                    to_drawing_color(p.color.ok_or("prstShdw")?).ok_or("prstShdw")?;
                Effect::PrstShdw(PresetShadowEffect {
                    preset: p.prst.ok_or("prstShdw")?.into(),
