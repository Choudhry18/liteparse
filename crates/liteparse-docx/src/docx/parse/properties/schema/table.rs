...
use super::measure::{TableMeasureXml, deserialize_optional_nonnegative_table_measure};
...
pub(crate) struct TblPrXml {
    ...
    #[serde(
        rename = "tblStyleRowBandSize",
        default,
        deserialize_with = "crate::docx::parse::primitives::lenient::opt_val_attr"
    )]
    tbl_style_row_band_size: Option<u32>,
    #[serde(
        rename = "tblStyleColBandSize",
        default,
        deserialize_with = "crate::docx::parse::primitives::lenient::opt_val_attr"
    )]
    tbl_style_col_band_size: Option<u32>,
    ...
    tbl_overlap: Option<StTblOverlap>,
}
...
pub(crate) struct TblLookXml {
    /// A malformed bitfield leaves the explicit per-flag attributes to speak
    /// for themselves rather than failing the table.
    #[serde(
        rename = "@val",
        default,
...
impl TblPrXml {
    pub(crate) fn split(self) -> (TableProperties, Option<StyleId>) {
        ...
        let props = TableProperties {
            ...
            style_row_band_size: self.tbl_style_row_band_size,
            style_col_band_size: self.tbl_style_col_band_size,
            positioning: self.tblp_pr.map(Into::into),
            overlap: self.tbl_overlap.map(crate::docx::model::TableOverlap::from),
        };
        (props, style_id)
    }
}
...
impl From<TrPrXml> for TableRowProperties {
    fn from(x: TrPrXml) -> Self {
        Self {
            ...
            justification: x.jc.map(Alignment::from),
            cnf_style: x.cnf_style.map(CnfStyle::from),
            grid_before: x.grid_before.unwrap_or(0),
            w_before: x.w_before.map(Into::into),
            grid_after: x.grid_after.unwrap_or(0),
...
pub(crate) struct TcPrXml {
    ...
    text_direction: Option<StTextDirection>,
...
pub(crate) struct VMergeXml {
    /// An unrecognized `@val` degrades to the same "continue" that an absent
    /// one means, so a bad value merges the cell rather than failing the doc.
    #[serde(
        rename = "@val",
        default,
        deserialize_with = "crate::docx::parse::primitives::lenient::opt_attr"
...
impl From<TcPrXml> for TableCellProperties {
    fn from(x: TcPrXml) -> Self {
        Self {
            ...
            vertical_align: x.v_align.map(crate::docx::model::CellVerticalAlign::from),
            vertical_merge: x.v_merge.map(Into::into),
            grid_span: x.grid_span,
            text_direction: x
                .text_direction
                .map(crate::docx::model::TextDirection::from),
...
mod tests {
    ...
    fn negative_decimal_table_width_does_not_fail_the_table() {
        ...
        .expect("a negative table width must not fail the parse");
        let (tp, _) = parsed.split();
        assert_eq!(
            tp.alignment,
            Some(Alignment::Center),
            "sibling properties must survive"
        );
    }
