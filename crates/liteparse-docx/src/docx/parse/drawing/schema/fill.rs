    ...
    fn from(f: DrawingFillXml) -> Self {
        match f {
            ...
            DrawingFillXml::SolidFill(s) => match s.color {
                Some(c) => match to_drawing_color(c) {
                    Some(c) => Self::Solid(c),
                    None => Self::None,
                },
...
impl From<GradFillXml> for GradientFill {
    fn from(x: GradFillXml) -> Self {
        let stops = x
            .gs_lst
            .map(|l| l.stops.into_iter().filter_map(to_gradient_stop).collect())
            .unwrap_or_default();
...
/// A gradient stop with an unmodelled color element is dropped: `GradientStop`
/// requires a color and there is no honest placeholder for one.
fn to_gradient_stop(x: GsXml) -> Option<GradientStop> {
    Some(GradientStop {
        position: x.pos,
        color: to_drawing_color(x.color)?,
    })
}
...
impl From<PattFillXml> for PatternFill {
    fn from(x: PattFillXml) -> Self {
        Self {
            ...
            fg_color: x.fg_clr.and_then(|c| to_drawing_color(c.color)),
            bg_color: x.bg_clr.and_then(|c| to_drawing_color(c.color)),
        }
    }
}
