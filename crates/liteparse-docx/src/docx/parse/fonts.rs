...
pub fn parse_embedded_fonts(
    ...
    for embed_ref in refs {
        ...
        // Without a usable fontKey this one embedded font cannot be
        // de-obfuscated. Skip it — the document still renders with a
        // substituted face, which beats failing the whole parse.
        let key_bytes = match parse_font_key(&embed_ref.font_key) {
            Ok(k) => k,
            Err(e) => {
                log::warn!(
                    "skipping embedded font '{}': unusable fontKey: {e}",
                    embed_ref.family
                );
                continue;
            }
